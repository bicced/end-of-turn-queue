...
---
!`if [ -z "$ARGUMENTS" ]; then echo "Nothing to queue. Usage: /queue:add <prompt>"; else mkdir -p "${CLAUDE_PROJECT_DIR}/.claude" && jq -nc --arg p "$ARGUMENTS" '$p' >> "${CLAUDE_PROJECT_DIR}/.claude/prompt-queue" && echo "Queued for end-of-turn: $ARGUMENTS"; fi`
